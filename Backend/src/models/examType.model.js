...
const examTypeSchema = new mongoose.Schema({
  code: {
    ...
    match: [/^[A-Z0-9]+$/, 'Exam type code can only contain uppercase letters and numbers']
  },
  name: {
    type: String,
    unique: true,
  ...
  try {
    const examTypes = [
      ...
      },
      {
        ...
        description: 'Make-up exam for students who missed regular exams with valid reasons',
        weightage: 0
      },
      {
        code: 'SUPP', 
    ...
    logger.info(`ExamType seeding completed: ${result.upsertedCount} inserted, ${result.modifiedCount} modified`);
    return result;
  } catch (error) {
