...
eventAudienceSchema.statics.findEventsForTarget = async function (targetType, targetId) {
  return this.find({ targetType, targetId }).populate('eventId');
};

// ✅ Static Method: Add a Target Audience to an Event
eventAudienceSchema.statics.addEventAudience = async function (eventId, targetType, targetId) {
  return this.create({ eventId, targetType, targetId });
};

// ✅ Static Method: Remove an Audience Entry
eventAudienceSchema.statics.removeAudienceEntry = async function (eventId, targetType, targetId) {
  return this.deleteOne({ eventId, targetType, targetId });
};

// ✅ Static Method: Bulk Insert Audience for an Event
eventAudienceSchema.statics.addMultipleAudiences = async function (eventId, audiences) {
  const audienceDocs = audiences.map(({ targetType, targetId }) => ({
    eventId,
    targetType,
    targetId
  }));
  return this.insertMany(audienceDocs);
};
